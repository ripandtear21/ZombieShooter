...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public abstract class ItemSetManagerBase : MonoBehaviour
    {
        ...
        protected virtual void AddItem(Item item, IItemCategoryIdentifier category)
        {
            if (category == null) {
        ...
        /// <param name="category">The category that the item should be aded to.</param>
        protected virtual void AddItemSet(Item item, IItemCategoryIdentifier category)
        {
            AddItemSet(item, category, null);
        }
        ...
        /// </summary>
        /// <param name="item">The item to add the ItemSet for.</param>
        /// <param name="category">The category that the item should be added to.</param>
        /// <param name="ignoreItemSet">The callback to determine if the item set should be ignored.</param>
        protected void AddItemSet(Item item, IItemCategoryIdentifier category, Func<ItemSet,bool> ignoreItemSet)
        {
            ...
            for (int i = 0; i < m_CategoryItemSets.Length; ++i) {
                ...
                for (int j = 0; j < itemSetList.Count; ++j) {
                    if (ignoreItemSet?.Invoke(itemSetList[j]) ?? false) {
                        continue;
                    }
            ...
            if (addItemSet) {
                ...
                if (item.ItemDefinition.GetParent() != null) {
                    ...
                    for (int i = 0; i < itemSetList.Count; ++i) {
                        if (ignoreItemSet?.Invoke(itemSetList[i]) ?? false) {
                            continue;
                        }
        ...
        /// <param name="possibleParent">The ItemDefinition that may be a parent of the specified child.</param>
        ...
        {
            AddItemSet(itemSet, defaultItemSet, item.ItemDefinition.GetItemCategory());
        }
        ...
        private void AddItemSet( ItemSet itemSet, bool defaultItemSet, IItemCategoryIdentifier category)
        {
            // The category can have multiple parents. Keep trying to add the ItemSet to all of the possible categories.
            var categoryParents = category.GetDirectParents();
            if (categoryParents != null) {
                for (int i = 0; i < categoryParents.Count; ++i) {
                    AddItemSet(itemSet, defaultItemSet, categoryParents[i]);
                }
            }
        ...
        public void AddItemSet(ItemSet itemSet, bool defaultItemSet, IItemCategoryIdentifier category, bool addParents)
        {
            ...
            for (int i = 0; i < itemSetList.Count; ++i) {
                ...
                var slotLength = Mathf.Min(itemSet.Slots.Length, itemSetList[i].Slots.Length);
        ...
        /// </summary>
        /// <param name="categoryIndex">The index of the category to get the target ItemSet index of.</param>
        ...
        public void UpdateActiveItemSet(int categoryIndex, int itemSetIndex)
        {
...
#if DEBUG_ITEM
            Debug.Log($"Update Active Item Set: category index {categoryIndex}, active item set {activeItemSetIndex}, new item set index {itemSetIndex}");
#endif
            ...
            if (activeItemSetIndex != -1) {
                ...
                if (activeItemSetIndex < m_CategoryItemSets[categoryIndex].ItemSetList.Count) {
                    m_CategoryItemSets[categoryIndex].ItemSetList[activeItemSetIndex].Active = false;
                }
            }
            ...
            if (itemSetIndex != -1) {
                if (!string.IsNullOrEmpty((state = m_CategoryItemSets[categoryIndex].ItemSetList[itemSetIndex].State))) {
                    StateManager.SetState(m_GameObject, state, true);
            ...
            if (activeItemSetIndex != -1 && !string.IsNullOrEmpty((state = m_CategoryItemSets[categoryIndex].ItemSetList[activeItemSetIndex].State))) {
                ...
                if (string.IsNullOrEmpty(newState) || state != newState) {
                    StateManager.SetState(m_GameObject, state, false);
                }
            }
