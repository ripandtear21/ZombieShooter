    ...
    public abstract class CharacterLocomotion : StateBehavior, IForceObject
    {
        ...
        private void DeflectHorizontalCollisions()
        {
            ...
            for (int i = 0; i < hitCount; ++i) {
                ...
                var closestPoint = MathUtility.ClosestPointOnCollider(m_Transform, activeCollider, closestRaycastHit.point, m_MoveDirection, true, false);
                if ((Vector3.Dot(Vector3.Cross(Vector3.ProjectOnPlane(closestRaycastHit.point - closestPoint, m_Up).normalized, horizontalDirection).normalized, m_Up)) > 0) {
